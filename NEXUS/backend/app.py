from flask import Flask, request, render_template, redirect, flash, session, send_file
...
# === Chatbot Page ===
class LocalChatbot:
    def __init__(self, name="AI", model_name="mistral", personality="You are a helpful AI assistant.", history=None):
        self.name = name
...
def chatbot_route():
    ...
    # Load from session or set defaults
    chat_history = session.get("chat_history", [])
    personality = session.get("personality", "You are a helpful AI assistant.")

    # Update personality if changed
    if new_personality and new_personality != personality:
        personality = new_personality
        chat_history = []

    chatbot = LocalChatbot(personality=personality, history=chat_history)

    if request.method == "POST" and user_input:
        chatbot.get_response(user_input)

    # Save updated chat history and personality to session
    session["chat_history"] = chatbot.chat_history
    session["personality"] = chatbot.personality
